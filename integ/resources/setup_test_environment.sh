...
stackOutputs=$(aws cloudformation describe-stacks --region "$AWS_REGION" --stack-name integ-test-fluent-bit-${ARCHITECTURE}-${ECR_SUFFIX} --output text --query 'Stacks[0].Outputs[*].OutputValue')
